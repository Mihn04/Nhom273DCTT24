...
<body>
    <header>
        ...
        <div class="btn-dialog">
          <a href="#themkhachsan"><button>Thêm đất nước</button></a>
        </div>
    ...
    <div class="menu">
        ...
        <div class="tbdskhachsan">
          <h2>Danh sách đất nước</h2>
             <table class="table hover">
                <thead>
                    <tr>
                        <td class ="cot-name">Tên đất nước</td>
                        <td class ="cot-detail">Chi tiết</td>
                        <td class ="cot-picture">Ảnh</td>
                        <td class = "cot-ql">QL khách sạn</td>
                        <td class ="cot-ql">QL thành phố</td>
                ...
                <?php 
                   ...
                   if(isset($_POST['search'])&&($_POST!="")){
                    ...
                    $search = "";
                       ...
                       if ($search != '') {
                         $listquerry = "SELECT * FROM country WHERE nameCountry LIKE '%" . $search . "%'";
                       }
                        else 
                              $listquerry = "SELECT * FROM country"; 
                              $result = mysqli_query($conn, $listquerry);
                              while ($row = mysqli_fetch_array($result)){
                         ?>
                         <tbody>
                                  <tr>
                                     <td><?php echo $row["nameCountry"]; ?></td>
                                     <td><?php echo $row["detailCountry"]; ?></td>
                                     <td><img class ="tbimg" src="./picture/<?php echo $row["pictureCountry"]; ?>"></td>
                                     <td>
                                       <a href="qlkhachsan.php?xid=<?php echo $row['id'];?>" ><button class ="btn-table">Khách sạn</button></a> 
                                     </td>
                                     <td>
                                       <a href="qlthanhpho.php?xid=<?php echo $row['id'];?>" ><button class ="btn-table">Thành phố</button></a> 
                                     </td>
                                     <td>
                                       <a href="suadatnuoc.php?id=<?php echo $row['id'];?>"><button class = "btn-sua">Sửa</button></a> 
                                       <a onclick="return confirm('Bạn có chắc muốn xóa đất nước này không?');" href="xoadatnuoc.php?xid=<?php echo $row['id'];?>"><button class = "btn-xoa">Xóa</button></a>
                                     </td>
                                   </tr>
             ...
             <section class="dialog-themkhachsan" id="themkhachsan">
               ...
               <h2>Thêm đất nước</h2>
                 <div class="form-them">
                 <form action="themdatnuoc.php" method="post" enctype="multipart/form-data">
                     <div class="form-group">
                         <label for="nameCountry">Tên đất nước :</label>
                         <input required type="text" name = "nameCountry" class="form-control" placeholder="Nhập tên đất nước" id="nameCountry">
                       </div>
                       <div class="form-group">
                         <label for="detailCountry">Chi tiết:</label>
                         <input required type="text" name ="detailCountry" class="form-control" placeholder="Nhập chi tiết" id="detailCountry">
                       </div>
                    <div class="form-group">
                      <label for="pictureCountry">Ảnh :</label>
                      <input required type="file" name ="pictureCountry" class="form-control" placeholder="Nhập ảnh" id="pictureCountry">
                    </div>
              <div class="form-group">

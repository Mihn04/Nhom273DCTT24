...
<body>
    ...
    if (isset($_GET["month-year"] )) {
        ...
        $danh_sach_tk_ngay = "SELECT DAY(payday_Order_Tour) as payDayBill , SUM( total_price_Order_Tour) as total FROM Order_Tour WHERE YEAR(payday_Order_Tour) = $year && MONTH(payday_Order_Tour) =  $month GROUP BY DAY(payday_Order_Tour) ORDER BY payday_Order_Tour LIMIT 31";
    ...
    ?>
        <h2 class="h2-day">Doanh thu tour theo ngày của tháng <?php echo $month ?> năm <?php echo $year ?></h2>
    <div class="bieudo-container">
        ...
        for ($i = 0; $i < count($mang); $i++) {$chieudai = ($mang[$i]["total"] / $doanh_thu_cao_nhat) * 100;
            $maucot = 'green';

...
$danh_sach_tk_thang = "SELECT MONTH(payday_Order_Tour) as payMonthBill, SUM(total_price_Order_Tour) as total FROM Order_Tour WHERE YEAR(payday_Order_Tour) = $year GROUP BY MONTH(payday_Order_Tour) ORDER BY payday_Order_Tour LIMIT 12";
$result = mysqli_query($conn, $danh_sach_tk_thang);
if($result && mysqli_num_rows($result) > 0){
while ($row = mysqli_fetch_array($result)){
    $mang[] = $row;
}
?>
        <h2 class="h2-month">Doanh thu tour theo tháng của năm <?php echo $year ?></h2>
    ...
    <?php
        ...
        for ($i = 0; $i < count($mang); $i++) {
            ...
            <div class="cot" style="height: <?php echo $chieudai ?>%; background-color: <?php echo $maucot; ?>;" title="Tổng: <?php echo $mang[$i]["total"] ?> VND">
                <div class="cot-label">Tháng <?php echo $mang[$i]["payMonthBill"] ?></div>
            </div>
            <?php
        }
        ?>
        <?php
            ...
            echo '<h2 class="h2-month">Không có dữ liệu cho năm '.$year.' <h2>';
        }
        ?>
</section>
<script>
